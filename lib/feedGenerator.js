...
// Only the `style={{...}}` form is used in content today, so that is the one
// case translated; anything else capitalised is flagged loudly rather than
// silently shipped as broken markup.
...
const stripMdx = (markdown, slug) => {
  const withStyles = markdown.replace(
    /style=\{\{([^}]*)\}\}/g,
